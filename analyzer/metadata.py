...
class FileInfo:
    ...
    def to_ecs(self) -> Dict[str, Any]:
        ...
        extension = Path(self.name).suffix.lstrip('.') if '.' in self.name else ''
        ...
        if self.created_time:
            ecs_doc["file"]["created"] = self.created_time.isoformat() + "Z" if not self.created_time.isoformat().endswith('Z') else self.created_time.isoformat()
        if self.modified_time:
            ecs_doc["file"]["mtime"] = self.modified_time.isoformat() + "Z" if not self.modified_time.isoformat().endswith('Z') else self.modified_time.isoformat()
        if self.accessed_time:
            ecs_doc["file"]["accessed"] = self.accessed_time.isoformat() + "Z" if not self.accessed_time.isoformat().endswith('Z') else self.accessed_time.isoformat()
        
        # Add MIME type
        if self.mime_type:
            ecs_doc["file"]["mime_type"] = self.mime_type
        
        # Add file owner
        if self.owner:
            ecs_doc["file"]["owner"] = self.owner
        
        # Add file attributes (permissions)
        if self.attributes:
            ecs_doc["file"]["attributes"] = self.attributes
        ...
        if self.exe_company or self.exe_product or self.exe_description or self.exe_version:
            ecs_doc["file"]["pe"] = {}
            if self.exe_company:
                ecs_doc["file"]["pe"]["company"] = self.exe_company
            if self.exe_product:
                ecs_doc["file"]["pe"]["product"] = self.exe_product
            if self.exe_description:
                ecs_doc["file"]["pe"]["description"] = self.exe_description
            if self.exe_version:
                ecs_doc["file"]["pe"]["file_version"] = self.exe_version
        
        # Add code signature fields
        if self.is_signed is not None or self.sig_subject or self.sig_issuer:
            ecs_doc["file"]["code_signature"] = {}
            if self.is_signed is not None:
                ecs_doc["file"]["code_signature"]["exists"] = self.is_signed
            if self.sig_subject:
                ecs_doc["file"]["code_signature"]["subject_name"] = self.sig_subject
            if self.sig_issuer:
                ecs_doc["file"]["code_signature"]["issuer"] = self.sig_issuer
        ...
        if self.exe_domains or self.exe_ips:
            ecs_doc["related"] = {}
            if self.exe_domains:
                ecs_doc["related"]["domains"] = self.exe_domains
            if self.exe_ips:
                ecs_doc["related"]["ip"] = self.exe_ips
        
        # Add threat enrichment (VirusTotal, Defender)
        enrichments = []
        if self.vt_detected is not None:
            vt_enrichment = {
                "indicator": {
                    "type": "file"
                }
            ...
            if self.vt_link:
                vt_enrichment["indicator"]["reference"] = self.vt_link
            if self.vt_detection_ratio:
                vt_enrichment["matched"] = {
                    "atomic": self.vt_detection_ratio
                }
            enrichments.append(vt_enrichment)
        
        if self.defender_detected:
            defender_enrichment = {
                "indicator": {
                    "type": "file",
            ...
            if self.defender_threat_name:
                defender_enrichment["indicator"]["description"] = self.defender_threat_name
        ...
        if self.risk_score > 0:
            ecs_doc["event"]["risk_score"] = float(self.risk_score)
        
        # Add custom ArtifactSleuth namespace for fields not in ECS
        artifactsleuth = {}
        
        # Document fields
        if (self.doc_author or self.doc_last_modified_by or self.doc_title or 
            self.doc_subject or self.doc_company or self.doc_has_macros is not None or
            self.doc_has_javascript is not None or self.doc_suspicious_elements):
        ...
        # IOC fields
        if self.exe_urls or self.exe_suspicious_imports:
            artifactsleuth["ioc"] = {}
            if self.exe_urls:
                artifactsleuth["ioc"]["urls"] = self.exe_urls
            if self.exe_suspicious_imports:
                artifactsleuth["ioc"]["suspicious_imports"] = self.exe_suspicious_imports
        
        # Archive fields
        if self.is_archive or self.is_password_protected or self.archive_path:
            artifactsleuth["archive"] = {}
            if self.is_archive:
                artifactsleuth["archive"]["is_archive"] = self.is_archive
        ...
        if self.risk_reasons or self.extension_mismatch or self.expected_extensions:
            artifactsleuth["risk"] = {}
            if self.risk_reasons:
                artifactsleuth["risk"]["reasons"] = self.risk_reasons
            if self.extension_mismatch:
                artifactsleuth["risk"]["extension_mismatch"] = self.extension_mismatch
            if self.expected_extensions:
                artifactsleuth["risk"]["expected_extensions"] = self.expected_extensions
        
        # Add custom namespace if it has any data
        if artifactsleuth:
            ecs_doc["artifactsleuth"] = artifactsleuth
        
        # Add @timestamp for Elasticsearch
        if self.modified_time:
            ecs_doc["@timestamp"] = self.modified_time.isoformat() + "Z" if not self.modified_time.isoformat().endswith('Z') else self.modified_time.isoformat()
        else:
            ecs_doc["@timestamp"] = dt.utcnow().isoformat() + "Z"
